...
def extract_parts_to_json(
    input_file, output_file=None, start_tag="(** **** Exercise:", end_tag="[] *)"
):
    ...
    extracted_parts = [
        {f"exercise_{i}": match.strip()} for (i, match) in enumerate(matches)
    ]
    for i, extract in enumerate(extracted_parts):
        ...
        for i, part in enumerate(split_parts):
            ...
            if Definition_tag in last_line:
                ...
                name = extract_content_between_tags(
                    part, start_tag="Theorem ", end_tag=":"
                )
